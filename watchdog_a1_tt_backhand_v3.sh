...
EXP=logs/a1_tt_backhand_real_v3_realserve_predictor_r4dr
TARGET=19999
TASK=a1_tt_backhand_v3
TRAIN_LOG=train_a1_tt_backhand_real_v3_realserve_predictor_r4dr.log
WATCHDOG_LOG=train_a1_tt_backhand_real_v3_realserve_predictor_r4dr_watchdog.log
...
launch() {
  ...
  TRAIN_PID=$!
  echo "$TRAIN_PID" > train_a1_tt_backhand_v3_trainer.pid
  echo "[watchdog] $(date --iso-8601=seconds) trainer_pid=$TRAIN_PID" >> "$WATCHDOG_LOG"
}

echo "$$" > train_a1_tt_backhand_v3_watchdog.pid
if ! launch; then
  exit 0
fi
while true; do
