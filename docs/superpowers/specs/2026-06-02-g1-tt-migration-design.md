...
- **偏移轴 = 本地 +X**：G1 `right_wrist_roll_rubber_hand` 的 COM 在 xyz=(0.108,-0.0016,0.002)，前臂外伸轴为本地 +X（T1 是 -Y）。球拍管沿 +X 延伸，`local_offset` 用 +X。
- 打印件交付：同套参数后续用脚本导出 STL（管+套筒+卡口+空白安装面），硬件团队 mate 到 G1 腕部法兰。
- 限制说明：精确打印 CAD 需 G1 腕部机械接口尺寸（孔距/法兰），URDF 无此信息；本设计提供参数化结构件近似。

#### ②-bis 球拍质量 / 惯量（sim2real 关键参数）
- **强制**：URDF 里带碰撞的 link 必须显式 `<inertial>`（质量+质心+惯量张量），否则 PhysX 用默认值致手臂动力学错误。
- **替换而非叠加**：真机拆橡胶手换适配件 → 仿真直接**改写 `right_wrist_roll_rubber_hand` 的 `<inertial>`**（原 mass=0.357kg、COM=0.108m@+X），换成"适配件+真拍"，球拍几何也加在该 link（配合偏移法）。不双算。
- **质量取值**：真拍(底板+胶皮) ~0.17-0.20kg + PLA 管/套筒 ~0.05-0.10kg ≈ **0.22-0.30kg**（与原橡胶手 0.357kg 同量级，手臂动力学不剧变）。
- **质心/惯量**：建模为"细管(杆)+末端圆盘(拍面@~0.20m)"，质心比原手 0.108m 更靠外，绕腕/肘惯量增大（甩拍更重、击球动量更大，真实且想要）。给解析惯量张量，优于按均匀密度从网格算（真拍/空心管密度不均）。
- **打印后实测回填**：件子打出后称重+找平衡点，把实测 mass/COM 填回 URDF，取得最高保真；硬件就绪前用上述估值。
- **质量域随机化（建议）**：对球拍 mass/COM 加 ±20% 随机化（环境已randomize躯干质量±5kg），提升对真实器件实际重量的鲁棒性——运动链末端、直接影响击球，性价比高。
- 执行器余量：G1 臂电机 effort ~25Nm；0.25kg×0.20m 静态重力矩 ≈0.5Nm 可忽略，动态甩拍惯量亦远在限内。FK/动力学探针会顺带验证。
...
| `local_offset=[0,-0.345,0]` (`:857`) | 肘基 0.345m，沿 -Y | 腕基新偏移(~0.20m)沿 **+X**，FK 标定 |
...
| URDF 末端 link `<inertial>` | 橡胶手 0.357kg | 改写 `right_wrist_roll_rubber_hand` 为适配件+拍 ~0.22-0.30kg（杆+盘惯量）；可选 ±20% 质量随机化 |
