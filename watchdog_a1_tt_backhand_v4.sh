...
TARGET=19999
TASK=a1_tt_backhand_v4
TRAIN_LOG=train_a1_tt_backhand_real_v4_timing_return.log
WATCHDOG_LOG=train_a1_tt_backhand_real_v4_timing_return_watchdog.log
...
launch() {
  ...
  TRAIN_PID=$!
  echo "$TRAIN_PID" > train_a1_tt_backhand_v4_trainer.pid
  echo "[watchdog] $(date --iso-8601=seconds) trainer_pid=$TRAIN_PID" >> "$WATCHDOG_LOG"
}

echo "$$" > train_a1_tt_backhand_v4_watchdog.pid
if ! launch; then
  exit 0
fi
while true; do
