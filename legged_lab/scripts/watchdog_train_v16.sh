#!/usr/bin/env bash
# Watchdog for g1_tt_v16 — SPEED curriculum, WARM-STARTED from v12 model_10000.
#
...
#   logs/g1_tt_v16/seed_v14/, warm-start with a FRESH optimizer (LOAD_OPTIMIZER=0; reward+curriculum
...
TASK=g1_tt_dr
EXP=${EXP:-g1_tt_v16}
SEED_EXP=g1_tt_v14                       # warm-start source experiment
SEED_ITER=${SEED_ITER:-18000}            # warm-start source checkpoint iter (v14 last clean pre-critic-divergence)
NUM_ENVS=${NUM_ENVS:-4096}
TARGET=${TARGET:-50000}
LOGROOT="$REPO/logs/$EXP"
SEED_DIR="$LOGROOT/seed_v14"
WLOG="$REPO/train_${EXP}_watchdog.log"
cd "$REPO"

# --- one-time seed: copy v13 model_<SEED_ITER> into logs/g1_tt_v16/seed_v14/.
...
while true; do
  ...
  if [ "$N" -eq "$SEED_ITER" ] && [ "$DIR" = "seed_v14" ]; then
    export LOAD_OPTIMIZER=0
    echo "[wd] $(date +%F_%H-%M-%S) WARM-START from seed_v14/$FILE (iter $N) -> +$REM; LOAD_OPTIMIZER=0 TT_SIM_STEP_OFFSET=$TT_SIM_STEP_OFFSET" | tee -a "$WLOG"
  else
