#!/usr/bin/env bash
# Watchdog for a1_tt_damiao - A1 table-tennis scratch training with
# Damiao-style delayed PD and speed-torque envelope rewards.
#
# Usage (local):
#   NUM_ENVS=128 nohup bash legged_lab/scripts/watchdog_train_a1_damiao.sh > /tmp/a1_damiao_watchdog.log 2>&1 &
#
# Usage (cloud):
#   TARGET=100000 NUM_ENVS=4096 TRAIN_PY=/root/miniconda3/envs/pingpong/bin/python \
#     OMNI_KIT_ACCEPT_EULA=YES nohup bash legged_lab/scripts/watchdog_train_a1_damiao.sh >/dev/null 2>&1 &
set -u
...
TASK=${TASK:-a1_tt_damiao}
EXP=${EXP:-a1_tt_damiao_v1}
...
echo "[wd] $(date +%F_%H-%M-%S) start; target=$TARGET envs=$NUM_ENVS task=$TASK exp=$EXP (scratch, delayed PD, speed-torque rewards, no qdes limiter)" | tee -a "$WLOG"
while true; do
  ...
  echo "[wd] resume in 20s (to stop: kill $$ first, then pgrep -f '[a]1_tt_damiao')" | tee -a "$WLOG"
  sleep 20
done
