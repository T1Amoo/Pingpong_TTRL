"""Verify the complete A1 backhand-v2 reset-time serve contract on CPU."""
...
from legged_lab.physics import a1_backhand_v2_contract as contract
from legged_lab.physics.serve_flight import probe_serve_flight


def _lerp_pair(easy, hard, c):
    return (easy[0] + c * (hard[0] - easy[0]), easy[1] + c * (hard[1] - easy[1]))


def sample_candidates(count: int, curriculum: float):
    x_range = _lerp_pair(contract.EASY_BOUNCE_X_RANGE, contract.HARD_BOUNCE_X_RANGE, curriculum)
    vz_range = _lerp_pair(contract.EASY_BOUNCE_VZ_RANGE, contract.HARD_BOUNCE_VZ_RANGE, curriculum)
    y_center = contract.EASY_Y_CENTER + curriculum * (contract.HARD_Y_CENTER - contract.EASY_Y_CENTER)
    y_half = contract.EASY_Y_HALF + curriculum * (contract.HARD_Y_HALF - contract.EASY_Y_HALF)
...
def main():
    ...
    for curriculum in (0.0, 0.5, 1.0):
        ...
        print(
            f"[v2_serve] c={curriculum:.1f} candidates={args.samples} "
            f"acceptance={accepted / args.samples:.4f} accepted={accepted}"
        )
        for name in ("net_z", "bounce_x", "hit_y", "hit_z", "hit_vx", "hit_vz"):
            print(f"  {name} q0/q5/q50/q95/q100={quantiles(getattr(probe, name), valid)}")

        assert float(probe.net_z[valid].min()) >= contract.NET_CENTER_Z_MIN + contract.NET_PREDICTION_MARGIN - 1e-9
        assert float(probe.bounce_x[valid].min()) >= contract.PHYSICAL_BOUNCE_X_RANGE[0] - 1e-9
        assert float(probe.bounce_x[valid].max()) <= contract.PHYSICAL_BOUNCE_X_RANGE[1] + 1e-9
        assert float(probe.hit_y[valid].min()) >= contract.HIT_TARGET_Y_RANGE[0] - 1e-9
        assert float(probe.hit_y[valid].max()) <= contract.HIT_TARGET_Y_RANGE[1] + 1e-9
        assert float(probe.hit_z[valid].min()) >= contract.PREFLIGHT_HIT_Z_RANGE[0] - 1e-9
        assert float(probe.hit_z[valid].max()) <= contract.PREFLIGHT_HIT_Z_RANGE[1] + 1e-9
        assert float(probe.hit_vx[valid].abs().min()) >= contract.HIT_ARRIVAL_ABS_VX_RANGE[0] - 1e-9
        assert float(probe.hit_vx[valid].abs().max()) <= contract.HIT_ARRIVAL_ABS_VX_RANGE[1] + 1e-9
