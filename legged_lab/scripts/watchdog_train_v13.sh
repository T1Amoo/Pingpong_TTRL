#!/usr/bin/env bash
# Watchdog for g1_tt_v13 — SPEED curriculum, WARM-STARTED from v12 model_10000.
#
...
#   logs/g1_tt_v13/seed_v12/, warm-start with a FRESH optimizer (LOAD_OPTIMIZER=0; reward+curriculum
...
TASK=g1_tt_dr
EXP=${EXP:-g1_tt_v13}
SEED_EXP=g1_tt_v12                       # warm-start source experiment
SEED_ITER=${SEED_ITER:-10000}            # warm-start source checkpoint iter (v12 stage-1, pre-collapse)
NUM_ENVS=${NUM_ENVS:-4096}
TARGET=${TARGET:-30000}
LOGROOT="$REPO/logs/$EXP"
SEED_DIR="$LOGROOT/seed_v12"
WLOG="$REPO/train_${EXP}_watchdog.log"
cd "$REPO"

# --- one-time seed: copy v12 model_<SEED_ITER> into logs/g1_tt_v13/seed_v12/.
...
while true; do
  ...
  if [ "$N" -eq "$SEED_ITER" ] && [ "$DIR" = "seed_v12" ]; then
    export LOAD_OPTIMIZER=0
    echo "[wd] $(date +%F_%H-%M-%S) WARM-START from seed_v12/$FILE (iter $N) -> +$REM; LOAD_OPTIMIZER=0 TT_SIM_STEP_OFFSET=$TT_SIM_STEP_OFFSET" | tee -a "$WLOG"
  else
