#!/usr/bin/env bash
# Watchdog for g1_tt_v6 weekend training (from-scratch; stance -2.0, 30cm arm, ankle-overrun
# fix, real no-ball idle; 3-stage perf-gated curriculum).
# - Trains g1_tt (predictor-augmented, 4096 envs) up to TARGET iterations.
# - If training crashes / OOMs / is interrupted, auto-resumes from the numerically-latest ckpt.
# - To STOP for good: kill THIS watchdog's PID (forwards the kill to the child trainer).
#
# CURRICULUM-ON-RESUME: the idle/no-ball curriculum is keyed to env.sim_step_counter. On RESUME
#   this exports TT_SIM_STEP_OFFSET = N*240 (N = resumed iter) so the curriculum CONTINUES at the
#   right stage. (Serve difficulty is perf-gated -> serve_c resets to 0 on resume and re-ramps
#   by success; harmless.) 240 = decimation 10 * num_steps_per_env 24.
#
# Usage:
#   TRAIN_PY=/root/miniconda3/envs/pingpong/bin/python nohup bash legged_lab/scripts/watchdog_train_v6.sh > /dev/null 2>&1 &
#   echo $!            # watchdog PID; `kill <PID>` to stop everything
#   tail -f train_v6_watchdog.log
set -u
...
WLOG="$REPO/train_v6_watchdog.log"
cd "$REPO"
