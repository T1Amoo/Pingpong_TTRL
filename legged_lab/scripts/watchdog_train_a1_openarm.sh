#!/usr/bin/env bash
# Watchdog for a1_tt_openarm - A1 table-tennis scratch training with OpenArm-like
# implicit actuator limits and no q_des pre-limit/filter.
#
# Usage (local):
#   NUM_ENVS=128 nohup bash legged_lab/scripts/watchdog_train_a1_openarm.sh > /tmp/a1_openarm_watchdog.log 2>&1 &
#
# Usage (cloud):
#   TARGET=100000 NUM_ENVS=4096 TRAIN_PY=/root/miniconda3/envs/pingpong/bin/python \
#     OMNI_KIT_ACCEPT_EULA=YES nohup bash legged_lab/scripts/watchdog_train_a1_openarm.sh >/dev/null 2>&1 &
set -u

export OMNI_KIT_ACCEPT_EULA=${OMNI_KIT_ACCEPT_EULA:-YES}
unset A1_TT_EFFORT_SCALE
unset A1_TT_PROX_EFFORT_SCALE
unset A1_TT_DIST_EFFORT_SCALE

HERE="$(cd "$(dirname "$0")" && pwd)"
REPO="$(cd "$HERE/../.." && pwd)"
PY=${TRAIN_PY:-/home/woan/.conda/envs/pingpong/bin/python}
TASK=${TASK:-a1_tt_openarm}
EXP=${EXP:-a1_tt_openarm_v1}
...
echo "[wd] $(date +%F_%H-%M-%S) start; target=$TARGET envs=$NUM_ENVS task=$TASK exp=$EXP (scratch, openarm-like implicit actuator, no qdes limiter)" | tee -a "$WLOG"
while true; do
  ...
  echo "[wd] resume in 20s (to stop: kill $$ first, then pgrep -f '[a]1_tt_openarm')" | tee -a "$WLOG"
  sleep 20
done
